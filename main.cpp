...
void server() {
    ...
    inet_pton(AF_INET, "192.168.151.42", &hint.sin_addr);
    ...
    while (true)
    {
        ...
        if (command == "user") {
            ...
            if (command == "message") {
                ...
                currentUser.writeMessage(currentUser, mess);
            }
        }
...
void client() {
    ...
    int port = 54000;
    std::string ipAddress = "172.16.88.95";
    ...
    if (inputLS == 's' || inputLS == 'S') {
        ...
        if (!usernameMatched) {
            ...
            std::string line = "user-" + std::to_string(currentUser.getId()) + " " + currentUser.getUsername() + " " +
                    std::to_string(currentUser.getFriends()) + " " + std::to_string(currentUser.getMessages()) + " " + currentUser.getRegistrationDate() + "-" +
                    std::to_string(currentUser.getId()) + "-" + "0";
        ...
        while (infinityLoop) {
            ...
            switch (command) {
                ...
                case 2: {
                    ...
                    int num;
                    std::cin >> num;
                    std::cout << "id: " << vec[num - 1].getId() << "\nusername: " << vec[num - 1].getUsername() << "\nfriends: " << vec[num - 1].getFriends() << "\nmessages: " << vec[num - 1].getMessages() << "\nregistration Date: " << vec[num - 1].getRegistrationDate() << std::endl;
                }
                    break;
            }
        }
        ...
        while (infinityLoop) {
            ...
            switch (command) {
                ...
                case 2: {
                    ...
                    std::string newFriendId;
                    for (auto us: allUsersList) {
                        if (us.getUsername() == friendUsername) {
                            newFriendId = std::to_string(us.getId());
                            break;
                        }
                    }
                    line = "friend-add-" + std::to_string(currentUser.getId()) + "-" + newFriendId;
                    send(sock, line.c_str(), line.size() + 1, 0);
                }
                    break;
                case 3: {
                    ...
                    int choice;
                    std::cin >> choice;
                    line = "friend-remove-" + std::to_string(currentUser.getId()) + "-" + std::to_string(vec[choice - 1].getId());
                    send(sock, line.c_str(), line.size() + 1, 0);
                ...
                case 4: {
                    ...
                    line = "message-" + message + "-" + std::to_string(currentUser.getId()) + "-" + std::to_string(vec[choice - 1].getId());
                    send(sock, line.c_str(), line.size() + 1, 0);
...
std::vector<User> showFriendList(int currentId) {
    ...
    std::vector<User> vec;
    std::string path = "/home/mesasori/CLionProjects/laba3/friends_" + std::to_string(currentId) + ".txt";
